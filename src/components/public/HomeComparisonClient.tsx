...
export function HomeComparisonClient() {
  const { t, language } = useLanguage();

  return (
    <section className="py-20 bg-slate-100 dark:bg-slate-900/60 border-y border-slate-200 dark:border-slate-800 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-12">
          <h2 className="text-xs font-black text-teal-600 dark:text-teal-400 tracking-widest uppercase">
            {t("coreInnovation")}
          </h2>
          <h3 className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
            {t("compSectionTitle")}
          </h3>
        ...
        {/* High-Contrast Dynamic Interactive Table */}
        <div className="bg-white dark:bg-slate-950 rounded-2xl border border-teal-500/30 p-6 shadow-2xl overflow-x-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800 mb-4 text-xs font-semibold gap-2">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-rose-500" />
              <div className="w-3 h-3 rounded-full bg-amber-500" />
              <div className="w-3 h-3 rounded-full bg-teal-500" />
              <span className="font-mono text-slate-700 dark:text-slate-400 ml-2">
                {t("patientLabel")}: Alexander Wright (MRN-2026-8801)
              </span>
            </div>
            <div className="text-slate-700 dark:text-slate-400">
              {t("comparingReports")} <span className="text-slate-900 dark:text-slate-200 font-bold">LAB-2026-01045</span> {t("vsText")} <span className="text-teal-600 dark:text-teal-400 font-bold">LAB-2026-08001</span>
            </div>
          </div>
          ...
          <table className="w-full text-left text-xs font-mono">
            <thead>
              <tr className="text-slate-600 dark:text-slate-400 font-extrabold uppercase border-b border-slate-200 dark:border-slate-800 text-[10px]">
                <th className="py-3 px-4">{t("diagnosticTestCol")}</th>
                <th className="py-3 px-4">{t("janBaselineCol")}</th>
                <th className="py-3 px-4">{t("augCurrentCol")}</th>
                <th className="py-3 px-4">{t("unitLabel")}</th>
                <th className="py-3 px-4">{t("refRange")}</th>
                <th className="py-3 px-4">{t("absDeltaCol")}</th>
                <th className="py-3 px-4">{t("pctChangeCol")}</th>
                <th className="py-3 px-4">{t("flagStatus")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
              <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                <td className="py-3.5 px-4 font-sans font-bold text-slate-900 dark:text-slate-200">{t("wbcTestName")}</td>
              ...
              <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                <td className="py-3.5 px-4 font-sans font-bold text-slate-900 dark:text-slate-200">{t("spermTestName")}</td>
              ...
              <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                <td className="py-3.5 px-4 font-sans font-bold text-slate-900 dark:text-slate-200">{t("hbTestName")}</td>
