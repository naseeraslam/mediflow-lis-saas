...
import { Shield, Lock, UserPlus, Search, Dna, HeartPulse, Menu, X, BookOpen } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800/80 max-w-[100vw] overflow-x-hidden transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-2">
          {/* Logo & Brand Identity (Mobile Responsive) */}
          <Link href="/" className="flex items-center gap-2 group shrink-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-tr from-teal-500 via-emerald-400 to-sky-500 flex items-center justify-center text-slate-950 font-black shadow-md shadow-teal-500/20 group-hover:scale-105 transition-transform">
            ...
            <div>
              <div className="text-base sm:text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none flex items-center gap-1">
                <span>MediFlow</span>
                <span className="hidden sm:inline-flex text-teal-600 dark:text-teal-400 font-black">{t("platformTitle")}</span>
                <span className="hidden md:inline-flex text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded-full bg-teal-500/10 text-teal-700 dark:text-teal-300 border border-teal-500/30 font-extrabold uppercase tracking-wider">
                  {t("saasTag")}
                </span>
              </div>
          ...
          <nav className="hidden lg:flex items-center gap-6 text-sm font-bold text-slate-800 dark:text-slate-200">
            ...
            <Link href="/user-guide" className="hover:text-teal-600 dark:hover:text-teal-400 transition-colors flex items-center gap-1.5">
              <BookOpen className="w-4 h-4 text-teal-500" /> User Guide
            </Link>
          ...
          {/* Right Action Bar (Mobile Responsive & Zero Overflow) */}
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            >
              {mobileMenuOpen ? <X className="w-5 h-5 text-teal-600 dark:text-teal-400" /> : <Menu className="w-5 h-5 text-slate-800 dark:text-slate-200" />}
            </button>
          </div>
        </div>
      </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-200 dark:border-slate-800 bg-white/98 dark:bg-slate-950/98 backdrop-blur-2xl px-4 py-5 space-y-4 shadow-2xl animate-in slide-in-from-top-2 duration-200">
          <nav className="flex flex-col space-y-3 text-sm font-bold text-slate-800 dark:text-slate-200">
            <Link
              href="/patient-search"
              onClick={() => setMobileMenuOpen(false)}
              className="p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center gap-2.5"
            >
              <Search className="w-4.5 h-4.5 text-teal-600 dark:text-teal-400" /> {t("patientSearchPortal")}
            </Link>
            ...
            <Link
              ...
              className="p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center gap-2.5"
            >
              <BookOpen className="w-4.5 h-4.5 text-teal-500" /> User Guide & Manual
            </Link>

            <Link
              href="/pricing"
              onClick={() => setMobileMenuOpen(false)}
              className="p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900"
            >
              {t("pricing")}
            </Link>
            ...
            <Link
              ...
              className="p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center gap-2.5"
            >
              <Shield className="w-4.5 h-4.5 text-emerald-600 dark:text-emerald-400" /> {t("security")}
            </Link>
          </nav>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex flex-col gap-2.5">
            <Link
              href="/login"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full py-3 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100 font-extrabold text-xs flex items-center justify-center gap-2 shadow-sm"
            >
